...
//		 .forEach(System.out::println);
		...
		// long c = Person.persons().stream().filter(Person::isFemale).count();
		...
		// OptionalDouble income = Person.persons().stream()
...
//		SortedSet<Person> persons = Person.persons().stream()
//									// .map(Person::getName)
